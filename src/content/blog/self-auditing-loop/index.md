...
```text
        ...
        └───────────────────────────────┬───────────────────────────────┘
                                         │ đến giờ
                                         ▼
   _audit/requests/*.md ─────────►  lọc các request enabled cùng type
   (khai báo: cần soi gì)                │
                                         ▼   với từng request:
                          ┌──────────────────────────────┐
                          │  collector.sh --request X    │   THU THẬP
...
Scheduler không hard-code lịch. Nó đọc `_audit/cron-jobs.json`, mỗi dòng là một cặp *cron expression → loại audit*, rồi đăng ký từng job:
...
for (const job of this.cronJobs) {
  const cronJob = new CronJob(job.cron, () => {
    void this.runRequestBatch(job.auditType);   // đến giờ → chạy một mẻ
  });
...
Muốn đổi từ "4 tiếng một lần" sang "mỗi giờ", hay thêm một loại audit mới chạy theo nhịp riêng: sửa JSON, restart, xong. Không đụng vào một dòng TypeScript nào. Lịch là *cấu hình*, không phải *logic*.
...
Người đọc sau bài sẽ nhớ không phải `collector.sh` hay `cron-jobs.json`, mà là ba nguyên tắc:

> **Data collection nên deterministic. AI chỉ nên làm reasoning. Snapshot là hợp đồng giữa hai tầng.**

Đó mới là takeaway đủ mạnh của một bài blog kỹ thuật.
